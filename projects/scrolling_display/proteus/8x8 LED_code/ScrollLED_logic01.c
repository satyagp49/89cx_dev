...
unsigned short dummyCharData [8] = {0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01};
unsigned int DisplayBuffer[]={0, 0, 0, 0, 0, 0, 0, 0};
unsigned int speed;
short  l, k, ShiftAmount, scroll, temp, shift_step=2, StringLength;
        ...
        for (k=0; k<StringLength; k++){
			for (scroll=0; scroll<(8/shift_step); scroll++) {
				for (ShiftAmount=0; ShiftAmount<8; ShiftAmount++){
                    ...
                    select_line = (0xFF & ~(0x01 << ShiftAmount));
                    P3 = select_line;
                    delay_ms(20);
				}/*
				speed = 1;
